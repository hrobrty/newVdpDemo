...
- (void)transformLinphoneCoreToKeys {
    ...
    if (cfg){
        ...
        [self setObject:@"5000"   forKey:@"username_preference"];
        [self setObject:@"120.76.225.49"   forKey:@"domain_preference"];
        [self setObject:@"120.76.225.49"   forKey:@"proxy_preference"];
        [self setObject:@"legrand"   forKey:@"password_preference"];
...
//        [LinphoneLogger logc:LinphoneLoggerDebug format:"%@ IPV6", enable_ipv6?@"ENABLING":@"DISABLING"];
    ...
    //configure sip account 配置sip账户
    
    //mandatory parameters 必须参数
    NSString*        username = [self stringForKey:@"username_preference"];//用户名
    NSString*          userID = [self stringForKey:@"userid_preference"];//用户ID
    NSString*          domain = [self stringForKey:@"domain_preference"];//域名
    NSString*       transport = [self stringForKey:@"transport_preference"];//传输类型
    NSString*      accountHa1 = [self stringForKey:@"ha1_preference"];
    NSString* accountPassword = [self stringForKey:@"password_preference"];//账户密码
    ...
    if (username && [username length] >0 && domain && [domain length]>0) {
        ...
        // use proxy as route if outbound_proxy is enabled 如果开启了域外服务器,使用代理作为路由
        ...
        // add auth info 添加授权信息
        linphone_core_clear_all_auth_info(lc);
        if( info ) { linphone_core_add_auth_info(lc,info); }
        
        // setup new proxycfg 设置新的代理配置
        if( isEditing ){
            linphone_proxy_config_done(proxyCfg);
        } else {
            // was a new proxy config, add it 是新的代理配置,添加它
