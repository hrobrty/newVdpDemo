...
//开锁
- (void)unlock: (NSString *)calleeSipAccount domain:(NSString *)domain{
    NSString *message = @"title=unlock\n";
//    [message stringByAppendingFormat:@"title=%@\naccount=%@\ncallid=%@\n",title,account,callid];
//    snprintf(message, sizeof(message)-1, "title = %s\naccount=%s\ncallid=%s\n","unlock",account?account:@"",callid? callid:@"");
    NSString *uri =  [calleeSipAccount stringByAppendingFormat:@"@"];
    uri = [uri stringByAppendingString:domain];
    NSURL *url = nil;
    
    LinphoneChatRoom *cr = linphone_core_get_chat_room_from_uri([LinphoneManager getLc], uri.UTF8String);
    LinphoneChatMessage *msg = linphone_chat_room_create_message(cr, [message UTF8String]);
    linphone_chat_message_set_external_body_url(msg, [[url absoluteString] UTF8String]);
    
    // we must ref & unref message because in case of error, it will be destroy otherwise
    linphone_chat_room_send_chat_message(cr, linphone_chat_message_ref(msg));
...
- (int)takeSnapShot{
    ...
    const char *file;
//  int rets = linphone_call_take_preview_snapshot(call, file);
    int ret = linphone_call_take_video_snapshot(call, file);
    return ret;
}
//注销
- (void)unregisterSipaccount{
    ...
    linphone_core_remove_proxy_config(lc, proxyCfg);
}
...
//  linphone_call_params_set_audio_direction(lcallParams,LinphoneMediaDirectionSendRecv);
...
//		[LinphoneLogger logc:LinphoneLoggerError format:"GSM call in progress, cancelling outgoing SIP call request"];
  ...
  LinphoneCall* currentCall = linphone_core_get_current_call(theLinphoneCore);
	...
	//get default proxy
	linphone_core_get_default_proxy(theLinphoneCore,&proxyCfg);
	LinphoneCallParams* lcallParams =  linphone_core_create_call_params(theLinphoneCore,currentCall);
  linphone_call_params_enable_video(lcallParams,YES);
...
//        ortp_set_log_level_mask(ORTP_DEBUG|ORTP_MESSAGE|ORTP_WARNING|ORTP_ERROR|ORTP_FATAL,3);
